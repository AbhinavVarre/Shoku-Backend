...
def test_following(client, session):
    ...
    response = client.post(
        "/login", data={"username": user1.name, "password": user1.password}
    )
    assert response.status_code == 200
    ...
    # create list
    restaurant_list = schemas.RestaurantListCreate(
    ...
    response = client.post(
        "/login", data={"username": user2.name, "password": user2.password}
    )
    assert response.status_code == 200
    assert "access_token" in response.json()
    token = response.json()["access_token"]
