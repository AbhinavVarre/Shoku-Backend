...
#follow another user
@router.post("/follow/{user}", response_model=schemas.User, summary="Follow another user")
def follow_user(user: str, current_user: models.Users = Depends(oauth2.get_current_user), db: Session = Depends(get_db)):
...
@router.post("/unfollow/{user}", response_model=schemas.User, summary="Unfollow another user")
def unfollow_user(user: str, current_user: models.Users = Depends(oauth2.get_current_user), db: Session = Depends(get_db)):
    user = crud.get_user(db, name=user)
...
def get_followers(current_user: models.Users = Depends(oauth2.get_current_user), db: Session = Depends(get_db)):
    return current_user.followers.all()

#get following
@router.get("/following", response_model=list[schemas.User], summary="Get following")
def get_following(current_user: models.Users = Depends(oauth2.get_current_user), db: Session = Depends(get_db)):
