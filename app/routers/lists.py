...
def share_list(list_name: str, user_id:int, current_user_name: str = Depends(oauth2.get_current_user), db: Session = Depends(get_db)):
    list = read_list(db=db, list_name=list_name, current_user_name=current_user_name)
...
def read_list(list_name: str, current_user_name: str = Depends(oauth2.get_current_user), db: Session = Depends(get_db)):
    lists = read_all_lists(db=db, current_user_name=current_user_name)
